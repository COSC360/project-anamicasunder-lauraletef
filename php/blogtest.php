...
if (isset($_SESSION['username'])) {
   ...
   if (isset($_POST['submit'])) {
    ...
    // Insert the new post into the database
    $sql = "INSERT INTO comments (username, content, post_id) VALUES ('$username', '$content', '$post_id')";
    if ($conn->query($sql) === TRUE) {
        echo "<p>Post added successfully.</p>";
    } else {
        echo "Error: " . $sql . "<br>" . $conn->error;
...
// User is not logged in, display comments based on keyword search
if (isset($_POST['search'])) {
    $search_query = $_POST['search'];
    $sql = "SELECT DISTINCT * FROM commentas WHERE content LIKE '%$search_query%'";
    $result = $conn->query($sql);
} else {
...
if ($result->num_rows > 0) {
    // Display all the comments in the database
    echo '<form method="post" action="">
            <label for="search">Search by username or keyword:</label>
            <input type="text" name="search" id="search">
            <input type="submit" value="Search">
